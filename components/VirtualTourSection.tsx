...
const VirtualTourSection: React.FC = () => {
  ...
  const benefits = [
    { icon: <CheckCircle2 className="w-5 h-5 text-brand-primary" />, text: "Més confiança" },
    { icon: <CheckCircle2 className="w-5 h-5 text-brand-primary" />, text: "Més interès" },
    { icon: <CheckCircle2 className="w-5 h-5 text-brand-primary" />, text: "Més temps d'atenció" },
    { icon: <CheckCircle2 className="w-5 h-5 text-brand-primary" />, text: "Més possibilitats de venda" }
  ];

  const highlights = [
    { icon: <Eye size={18} />, text: "Explora abans d'arribar" },
    { icon: <Smartphone size={18} />, text: "El teu negoci obert 24/7" },
    { icon: <MousePointer2 size={18} />, text: "El client entra abans de decidir" }
  ];
          ...
          >
            ...
            <div className="p-6 md:p-8 bg-white/[0.03] rounded-[2rem] border border-white/5 mb-12 backdrop-blur-sm">
              ...
              <div className="grid sm:grid-cols-2 gap-4 md:gap-6">
                {(t('home_v3.virtual_tour.benefits', { returnObjects: true }) as any[] || benefits).map((benefit: any, idx: number) => (
            ...
            <div className="flex flex-wrap gap-3 md:gap-4 mb-12">
              {(t('home_v3.virtual_tour.highlights', { returnObjects: true }) as any[] || highlights).map((h: any, i: number) => (
            ...
            <div className="flex flex-col sm:flex-row gap-6 mb-20">
              ...
              >
                {t('launch.banner_button', 'Reserva la teva plaça')} <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
              </button>
            </div>
            ...
            >
              <div className="space-y-4">
                <h3 className="text-3xl md:text-5xl font-black uppercase tracking-tighter text-white">
                  Demo <span className="text-brand-primary">Interactiva</span>
                </h3>
              </div>
                ...
                <div className="space-y-4">
                  <div className="aspect-video rounded-[1.5rem] md:rounded-[2rem] overflow-hidden border border-white/10 bg-black shadow-2xl relative group">
                    ...
                    <div className="absolute top-4 left-4 z-10 flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-black/60 backdrop-blur-md border border-white/15">
                      ...
                      <span className="text-[10px] md:text-xs font-black uppercase tracking-wider text-white">Visualització 3D Automàtica</span>
                    </div>
