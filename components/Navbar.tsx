...
const Navbar: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const updateNavbarHeight = () => {
      if (navRef.current && (!isScrolled && !isDismissed)) {
  ...
  return (
    ...
    >
      ...
      <div 
        className={cn(
          "w-full bg-neutral-950/95 border-b border-white/10 backdrop-blur-xl px-3 sm:px-6 text-white shadow-lg relative z-[100] transition-all duration-300 ease-in-out overflow-hidden will-change-[max-height,opacity]",
          (isScrolled || isDismissed) 
            ? "max-h-0 opacity-0 py-0 border-none pointer-events-none" 
            : "max-h-[500px] opacity-100 py-2.5 sm:py-3"
        )}
      >
        <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row items-center justify-between gap-3 text-center lg:text-left relative pr-6 sm:pr-0">
          <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-3 text-left">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-primary/20 border border-brand-primary/40 text-brand-primary text-xs md:text-sm font-mono font-extrabold tracking-wide shrink-0 shadow-[0_0_15px_rgba(0,82,255,0.3)]">
              <span className="text-sm md:text-base">🚀</span>
              <span>{t('launch.banner_title', 'Obertura de SKIRION')}</span>
            </div>
            <span className="text-xs sm:text-sm md:text-base text-gray-100 font-normal sm:font-medium leading-relaxed max-w-4xl tracking-tight text-center sm:text-left">
              {t('launch.banner_text', 'Estem seleccionant les empreses que implementaran els nostres sistemes. Cada projecte inclou una activació i un seguiment personalitzat, per això les implementacions es realitzaran de manera progressiva.')}
            </span>
          </div>
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            >
              <span>{t('launch.banner_button', 'Reserva la teva plaça')}</span>
            </button>
            <button
              type="button"
              onClick={() => setIsDismissed(true)}
              aria-label="Dismiss banner"
              className="p-1 sm:p-1.5 rounded-full text-gray-400 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
            >
              <X size={16} />
            </button>
          </div>
      ...
      <div className="w-full bg-neutral-950/95 border-b border-white/10 backdrop-blur-2xl px-3 sm:px-6 py-2 sm:py-3 shadow-2xl relative z-[100]">
        ...
        <div className="flex flex-col items-start gap-1">
          <Link to="/" className="flex items-center space-x-3 md:space-x-5 group shrink-0">
            <div className="w-14 h-14 md:w-24 md:h-24 bg-white/10 backdrop-blur-md rounded-2xl md:rounded-[2rem] flex items-center justify-center overflow-hidden border border-white/20 shadow-[0_0_50px_rgba(0,82,255,0.4)] group-hover:scale-110 transition-transform duration-500">
              <img 
